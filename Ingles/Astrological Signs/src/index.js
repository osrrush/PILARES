 ...
 */
let vocabulary = ['Aries', 'Taurus', 'Gemini','Cancer', 'Leo',  'Virgo', 'Libra',
                    'Scorpius', 'Sagittarius', 'Capricorn','Aquarius','Pisces'];
let vocabularyS = ['Aries', 'Tauro', 'Géminis', 'Cáncer', 'Leo', 'Virgo','Libra',
                    'Escorpio', 'Sagitario','Capricornio', 'Acuario', 'Piscis'];

const titulo = 'Astrological signs'; //Titulo del juego
const menulabel = [['Vocabulary'],
    ['Read and choose'],
    ['Listen and choose'],
    ['Listen and write']]; //Opciones menú

const Exp='Escoge el signo.';
	...
	this.add.graphics().lineStyle(2, 0xff0000).strokeRectShape(ini);
        ...
        for( var i=0 ; i < vocabulary.length ; i++ ){
            ...
            if (tam>310){
                var f = 290*80/tam;
                console.log(f);
                this.add.text(x+(i%5)*364+364/2 , y+110*Math.floor(i/5)+100/2, vocabularyS[i], { color: 'black', fontFamily: 'Arial', fontSize: f+'px '}).setOrigin(0.5,0.5);
            }else{
                this.add.text(x+(i%5)*364+364/2 , y+110*Math.floor(i/5)+100/2, vocabularyS[i], { color: 'black', fontFamily: 'Arial', fontSize: '80px '}).setOrigin(0.5,0.5);
            }
...
class RC extends Phaser.Scene{
    ...
    preload(){
        ...
        this.vocabulary2=vocabularyS.slice();
    ...
    over(imagen){
        ...
        this.lienzo.fillStyle(0xff0000);
        this.lienzo.setAlpha(0.5);
        this.lienzo.fillRect(50+364*x,260+110*y,364,110);
    ...
    clic(imagen){
        ...
        var x= Math.floor((imagen.x-50)/364);
        var y= Math.floor((imagen.y-260)/110);
        if(vocabularyS[5*y+x]===this.vocabulary2[rc]){
            b=0.5;
            c.play();
            rc++;
...
class LC extends Phaser.Scene{
    ...
    clic(imagen){
        ...
        var x= Math.floor((imagen.x-50)/364);
        var y= Math.floor((imagen.y-260)/110);
        if(vocabulary[5*y+x]===this.vocabulary2[rc]){
            b=0.5;
            c.play();
            rc++;
            rc=rc%vocabulary.length;
