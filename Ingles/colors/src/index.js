...
let vocabulary = ['red','orange','yellow','green','blue','purple','pink',
                    'white','grey','brown','black'];
...
class Explorar extends Phaser.Scene{
    ...
    create(){
        ...
        this.div.innerText = 'Pick a color.';
	...
	this.add.graphics().lineStyle(2, 0xff0000).strokeRectShape(ini);
        ...
        for( var i=0 ; i < vocabulary.length ; i++ ){
            ...
            this.graph2.fillStyle(0x000000);
            this.graph2.fillRect(x+(i%10)*170 , y+170*Math.floor(i/10),156,156);
            this.graph2.fillStyle(colors[i]);
            this.graph2.fillRect(x+3+(i%10)*170 , y+3+170*Math.floor(i/10),150,150);
...
class RC extends Phaser.Scene{
    ...
    clic(imagen){
        ...
        var x= Math.floor((imagen.x-20)/170);
        var y= Math.floor((imagen.y-220)/170);
        if(vocabulary[10*y+x]===this.vocabulary2[rc]){
            b=0.5;
            c.play();
            rc++;
...
class SW extends Phaser.Scene{
    ...
    preload(){
        ...
        this.indice.sort(() => (Math.random() > 0.5 ? 1 : -1));
	...
	this.add.graphics().lineStyle(2, 0xff0000).strokeRectShape(ini);
        ...
        this.image= this.add.image(1200,300, 'blanco');
        ...
        this.graph2 = this.add.graphics();
            ...
            this.graph2.fillStyle(0x000000);
            this.graph2.fillRect(1120 , 220,156,156);
            console.log(this.indice);
            console.log(this.indice[0]);
            this.graph2.fillStyle(colors[this.indice[rc]]);
            this.graph2.fillRect(1123 , 223,150,150);
    ...
    corregir(){
        ...
        console.log(vocabulary[this.indice[rc%vocabulary.length]]);
        var resp = vocabulary[this.indice[rc%vocabulary.length]];
        var rempl = resp.split('/');
        var resp = resp.split('/');
        ...
        for(var i=0;i<resp.length;i++){
            if(r === resp[i]){
                ...
                console.log(this.indice);
                console.log(this.indice[0]);
                this.graph2.fillStyle(colors[this.indice[rc%vocabulary.length]]);
                this.graph2.fillRect(1123 , 223,150,150);
                document.getElementById('respuesta').value="";
                break;
...
const config = {
  ...
  scene: [menu,SW,RC,LW,LC,Explorar ],
  scale: {
